...
//这条import很重要，否则swift-h报错
#import "AVKit/AVKit.h"
#import "shadowsocks-Swift.h"
...
- (UIEdgeInsets) getSafeAreas {
    SWBAppDelegate *delegate = [UIApplication sharedApplication].delegate;
...
- (void)viewDidLoad {
    ...
    self.tabBar = [[SWBTabBarView alloc] initWithFrame:CGRectMake(0, bounds.size.height - kTabBarHeight - [self getSafeAreas].bottom, bounds.size.width, kTabBarHeight)];
...
- (void)relayout:(CGRect)bounds {
    CGRect addrBarRect = CGRectMake(0, [self getSafeAreas].top, bounds.size.width, kToolBarHeight);
    CGRect webViewContainerRect = CGRectMake(0, [self getSafeAreas].top + kToolBarHeight, bounds.size.width, bounds.size.height - kToolBarHeight - kTabBarHeight - [self getSafeAreas].top - [self getSafeAreas].bottom);
    CGRect tabBarRect = CGRectMake(0, bounds.size.height - kTabBarHeight - [self getSafeAreas].bottom, bounds.size.width, kTabBarHeight);
...
- (void)updateWebViewTitle:(WKWebView *)webView {
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    if ([[[navigationAction.request URL] absoluteString] caseInsensitiveCompare:kNewTabAddress] == NSOrderedSame) {
//        [self openLinkInNewTab:[(SWBWebView *)webView lastClickedLink]];
    }
    else {
        if (navigationAction.navigationType == UIWebViewNavigationTypeLinkClicked ||
            navigationAction.navigationType == UIWebViewNavigationTypeBackForward) {
            SWBPage *page = [_pageManager pageByTag:webView.tag];
...
//    if ([[[[webView request] URL] absoluteString] caseInsensitiveCompare:[(SWBWebView *) webView locationHref]] == NSOrderedSame) {
    if (TRUE) {
        ...
        NSString *title = [((SWBWebView *) webView) title];
        _urlField.text = url;
        SWBPage *page = [_pageManager pageByTag:webView.tag];
        page.url = url;
        page.title = title;
...
- (void)showTestPipVC {
    TestPictureInPictureViewController *vc = [[TestPictureInPictureViewController alloc] init];
...
- (void)webViewContainerWebViewDidSwitchToWebView:(SWBWebView *)webView {
    [self resetTabBarButtonsStatus];
    [self syncPageManagerSelectionStatusWithSelectedTag:webView.tag];
    _urlField.text = webView.URL.absoluteString;
